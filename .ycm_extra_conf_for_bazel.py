...
def InitBazelConfig():
    ...
    These values are not expected to change during the session.
    """
    global WORKSPACE_PATH
    global EXECUTION_ROOT
    EXECUTION_ROOT = subprocess.check_output(
        ['bazel', 'info', 'execution_root']).decode('utf-8').strip()
    WORKSPACE_PATH = subprocess.check_output(
        ['bazel', 'info', 'workspace']).decode('utf-8').strip()
...
def Settings(**kwargs):
    ...
    try:
        action = subprocess.check_output(
            BAZEL_PRINT_FLAGS + [filename],
            stderr=subprocess.STDOUT,
            cwd=WORKSPACE_PATH).decode('utf-8')
